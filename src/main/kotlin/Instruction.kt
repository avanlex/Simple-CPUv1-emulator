enum class Instruction(value: Byte) {
    ...
    In this instruction syntax
    X = Not used,
    K = Constant,
    A = Instruction Address,
    P = Data Address.
    The complexity of an instruction is also defined by its addressing mode
    i.e. not just how much number crunching it does, but how it fetches its operands (data).
    Again, to simplify the required hardware these instructions are
    limited to simple addressing modes:

            Immediate : operand KK, a constant value,
                this is immediately available as it is stored in the instruction register
                i.e. part of the instruction read during the fetch phase.

            Absolute : operand AA or PP, an address in memory.
                Again, the address is stored in the instruction register,
                either specifying where the data to be processed is stored
                i.e. an INPUT instruction, or where a result
                produced should be stored in memory i.e. an OUTPUT instruction.
                Also used to define the address in memory of
